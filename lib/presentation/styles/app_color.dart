import 'package:flutter/material.dart';

class AppColor {
  static const Color primary = const Color(0xff0062BD);
  static const Color darkGrey = const Color(0xff525252);
  static const Color grey = const Color(0xff737477);
  static const Color text = const Color(0xff464646);
  static const Color lightGrey = const Color(0xffB1B1B1);
  static const Color black = const Color(0xff000000);

  // new colors
  static const Color darkPrimary = const Color(0xffd17d11);
  static const Color lightPrimary = const Color(0xCCd17d11); // color with 80% opacity
  static const Color grey1 = const Color(0xff707070);
  static const Color grey2 = const Color(0xff797979);
  static const Color  white = const Color(0xffFFFFFF);
  static const Color error = const Color(0xffe61f34); // red color
}